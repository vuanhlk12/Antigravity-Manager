...
const MODEL_GROUPS = {
  CLAUDE: ["claude-opus-4-6-thinking", "claude"],
  GEMINI_PRO: [
    "gemini-3.1-pro-high",
    "gemini-3.1-pro-low",
...
function isModelProtected(
  protectedModels: string[] | undefined,
  modelName: string,
): boolean {
...
function AccountRowContent({
  ...
  const validationBlockedLabel = getValidationBlockedStatusLabel(
  ...
  const displayModels = sortModels(
    (showAllQuotas
      ? (account.quota?.models || []).map((m) => {
          ...
          const label =
            ...
            (config?.i18nKey
              ? t(config.i18nKey)
              : config?.shortLabel || config?.label || m.name);
          return {
      ...
      : (pinnedModels
          .map((modelId) => {
            const m = account.quota?.models.find(
              (m) =>
                m.name === modelId ||
                getModelAliases(modelId).includes(m.name.toLowerCase()),
            );
            ...
            const label =
              ...
              (config?.i18nKey
                ? t(config.i18nKey)
                : config?.shortLabel || config?.label || modelId);
            return {
              id: modelId,
              label: label,
      ...
      // 过滤特定的 Claude/Gemini 思考变体 (仅在 show all 模式下隐藏；pinned 模式下用户显式固定的保留)
      if (showAllQuotas && m.id.includes("thinking")) return false;
    ...
    const labelKey = `${m.label}-${m.protectedKey}`;
    return (
      self.findIndex((t) => `${t.label}-${t.protectedKey}` === labelKey) ===
      index
    );
  });
      ...
      <td className="px-2 py-1 align-middle">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {isDisabled && (
              <span className="px-2 py-0.5 rounded-md bg-rose-100 dark:bg-rose-900/50 text-rose-700 dark:text-rose-300 text-[10px] font-bold flex items-center gap-1 shadow-sm border border-rose-200/50">
            ...
            {account.proxy_disabled && (
              <span className="px-2 py-0.5 rounded-md bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-300 text-[10px] font-bold flex items-center gap-1 shadow-sm border border-orange-200/50">
                <Ban className="w-2.5 h-2.5" />
            ...
            >
              ...
              <span
                className={cn(
                  "text-[11px] font-bold",
                  account.validation_blocked
                    ? "text-amber-700/80 dark:text-amber-400"
      ...
      >
        <div className="flex flex-wrap items-center justify-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity max-w-[180px] mx-auto">
          ...
          <button
            ...
            title={
              isDisabled
                ? t("accounts.disabled_tooltip")
                : isSwitching
                  ? t("common.loading")
                  : t(
                      "accounts.switch_to_classic",
                      "切换到 Antigravity (经典版)",
                    )
            }
            ...
            <button
              ...
              title={
                isDisabled
                  ...
                  : isRefreshing
                    ? t("common.loading")
                    : t("accounts.warmup_this", "预热该账号")
              }
              disabled={isRefreshing || isDisabled}
            >
              <Sparkles
                className={`w-3.5 h-3.5 ${isRefreshing ? "animate-pulse" : ""}`}
              />
            </button>
          )}
          <button
          ...
          <button
            ...
            title={
              account.proxy_disabled
                ? t("accounts.enable_proxy")
                : t("accounts.disable_proxy")
            }
          >
            {account.proxy_disabled ? (
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100 dark:border-base-200 bg-gray-50 dark:bg-base-200">
              ...
              <th className="px-2 py-1 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap sticky right-0 w-[180px] bg-gray-50 dark:bg-base-200 z-20 shadow-[-12px_0_12px_-12px_rgba(0,0,0,0.1)] dark:shadow-[-12px_0_12px_-12px_rgba(255,255,255,0.05)] text-center">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-base-200">
              {accounts.map((account) => (
                <SortableAccountRow
                  ...
                  onSwitch={(targetIde?: string) =>
                    onSwitch(account.id, targetIde)
                  }
                  ...
                  onUpdateLabel={
                    onUpdateLabel
                      ? (label: string) => onUpdateLabel(account.id, label)
                      : undefined
                  }
