  ...
  const displayModels = useMemo(() => {
    ...
    if (showAllQuotas) {
      ...
      if (pinned && pinned.length > 0) {
        ...
        models = accountModels.filter(
          (m) =>
            DEFAULT_MODELS.some((d) => d.id === m.id) || m.data.display_name,
        );
      }
    }

    // 应用排序并过滤过期模型（thinking 变体仅在 show all 模式下隐藏）
    return sortModels(models).filter((m) => {
      if (!showAllQuotas) return true; // pinned 模式下显示用户显式固定的所有模型
      return (
        m.id !== "claude-sonnet-4-6-thinking" &&
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-1.5">
          ...
          <div className="flex items-center justify-between w-full gap-2">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {isDisabled && (
                <span className="px-1.5 py-0.5 rounded-md bg-rose-100 dark:bg-rose-900/40 text-rose-700 dark:text-rose-300 text-[9px] font-bold flex items-center gap-1 shadow-sm border border-rose-200/50">
                  <Ban className="w-2.5 h-2.5" />
                  {t("accounts.disabled").toUpperCase()}
                </span>
              )}
              {account.proxy_disabled && (
                <span className="px-1.5 py-0.5 rounded-md bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300 text-[9px] font-bold flex items-center gap-1 shadow-sm border border-orange-200/50">
                  <Ban className="w-2.5 h-2.5" />
        ...
        account.validation_blocked ? (
          <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 h-full py-4 text-center">
            ...
            >
              ...
              ) : isDisabled || account.proxy_disabled ? (
      ...
      <div className="flex-none flex items-center justify-center pt-2 pb-1 border-t border-gray-100 dark:border-base-200">
          ...
          <div className="absolute inset-0 bg-white/95 dark:bg-base-100/95 rounded-xl z-10 flex items-center justify-center p-4">
            <div className="flex items-center gap-2 w-full max-w-xs">
              <input
                ...
                placeholder={t(
                  "accounts.custom_label_placeholder",
                  "Enter custom label",
                )}
                value={labelInput}
        ...
        <div className="flex flex-wrap items-center justify-center gap-1 w-full">
          ...
          <button
            ...
            disabled={isRefreshing || isDisabled}
            title={
              isDisabled ? t("accounts.disabled_tooltip") : t("common.refresh")
            }
          >
            <RefreshCw
